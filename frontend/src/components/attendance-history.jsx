...
export function AttendanceHistory() {
  ...
  const getStatus = (percentage) => {
    if (percentage >= 90)
      return { label: "Excellent", color: "bg-green-400/10 text-green-400" };
    if (percentage >= 75)
      return { label: "Good", color: "bg-blue-400/10 text-blue-400" };
    if (percentage >= 50)
      return { label: "Warning", color: "bg-yellow-400/10 text-yellow-400" };
  ...
  const calculateRequiredClasses = (attendanceArray) => {
    ...
    const attendedClasses = attendanceArray.filter(
      (date) => date.status === "Present" || date.status === "DutyLeave"
    ).length;

    const targetAttendance = 0.75; // 75%
  ...
  if (loading) {
    return (
      <Card className="bg-zinc-900 border-zinc-800 p-6 text-center text-white">
